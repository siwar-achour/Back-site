<template>
  <div>
    <h2 id="page-heading" data-cy="AdminHeading">
      <span v-text="$t('projetPfeApp.admin.home.title')" id="admin-heading">Admins</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('projetPfeApp.admin.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'AdminCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('projetPfeApp.admin.home.createLabel')"> Create a new Admin </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && admins && admins.length === 0">
      <span v-text="$t('projetPfeApp.admin.home.notFound')">No admins found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="admins">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('projetPfeApp.admin.roleName')">Role Name</span></th>
            <th scope="row"><span v-text="$t('projetPfeApp.admin.userName')">User Name</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="projetPfeApp.admin.delete.question" data-cy="adminDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-admin-heading" v-text="$t('projetPfeApp.admin.delete.question', { id: removeId })">
