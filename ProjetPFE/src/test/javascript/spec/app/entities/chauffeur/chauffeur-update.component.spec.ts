/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import Router from 'vue-router';
import { ToastPlugin } from 'bootstrap-vue';

import * as config from '@/shared/config/config';
import ChauffeurUpdateComponent from '@/entities/chauffeur/chauffeur-update.vue';
import ChauffeurClass from '@/entities/chauffeur/chauffeur-update.component';
import ChauffeurService from '@/entities/chauffeur/chauffeur.service';

import AdminService from '@/entities/admin/admin.service';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
const router = new Router();
localVue.use(Router);
localVue.use(ToastPlugin);
localVue.component('font-awesome-icon', {});
localVue.component('b-input-group', {});
localVue.component('b-input-group-prepend', {});
localVue.component('b-form-datepicker', {});
localVue.component('b-form-input', {});

describe('Component Tests', () => {
  describe('Chauffeur Management Update Component', () => {
    let wrapper: Wrapper<ChauffeurClass>;
    let comp: ChauffeurClass;
    let chauffeurServiceStub: SinonStubbedInstance<ChauffeurService>;

    beforeEach(() => {
      chauffeurServiceStub = sinon.createStubInstance<ChauffeurService>(ChauffeurService);

      wrapper = shallowMount<ChauffeurClass>(ChauffeurUpdateComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: {
          chauffeurService: () => chauffeurServiceStub,
          alertService: () => new AlertService(),

          adminService: () =>
            sinon.createStubInstance<AdminService>(AdminService, {
              retrieve: sinon.stub().resolves({}),
            } as any),
        },
      });
      comp = wrapper.vm;
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', async () => {
        // GIVEN
        const entity = { id: 'ABC' };
        comp.chauffeur = entity;
        chauffeurServiceStub.update.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(chauffeurServiceStub.update.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', async () => {
        // GIVEN
        const entity = {};
        comp.chauffeur = entity;
        chauffeurServiceStub.create.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(chauffeurServiceStub.create.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundChauffeur = { id: 'ABC' };
        chauffeurServiceStub.find.resolves(foundChauffeur);
        chauffeurServiceStub.retrieve.resolves([foundChauffeur]);

        // WHEN
        comp.beforeRouteEnter({ params: { chauffeurId: 'ABC' } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.chauffeur).toBe(foundChauffeur);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
